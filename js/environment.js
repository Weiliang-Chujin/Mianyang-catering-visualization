...
d3.json("data/useritem.json",function(error,da){
	...
	arcs.append("text")
		...
		.text(function(d){
			var percent = Number(d.value)/d3.sum(dataEnvironmentsum)*100;
	...
	//添加一个提示框
			var tooltip=d3.select("#environment")
						.append("div")
						.attr("class","tooltip")
						.style("opacity",0.0);
			...
			tooltip.html("有"+d.value+"条评论给其"+"<br />"+"消费店铺环境评分为"+i+"，占比"+percent.toFixed(0)+'%')
